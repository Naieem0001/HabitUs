    ...
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <nav className="sticky top-0 z-40 backdrop-blur-xl border-b border-border bg-background/70">
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <a href="/dashboard" className="text-lg font-bold flex items-center gap-2 text-foreground">
      ...
      <main className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          <h1 className="text-4xl sm:text-5xl font-bold leading-tight mb-2">
            Welcome back, <span className="text-gradient-amber">{user?.user_metadata?.full_name?.split(' ')[0] || 'Challenger'}</span>.
          </h1>
          <p className="text-muted-foreground text-lg">Your squad is ranking high. Keep the streak alive! 🔥</p>
        </motion.div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {task.priority && (
                              <span className={`px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap border ${getPriorityColor(task.priority).bg} ${getPriorityColor(task.priority).text} ${getPriorityColor(task.priority).border}`}>
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <div className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs font-bold flex-shrink-0 ${
                            idx === 0 ? 'gradient-amber text-white' : 'bg-surface-2 text-foreground'
                          }`}>
                            {idx + 1}
                          </div>
      ...
      {showProofModal && selectedTask && (
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-4">Challenge: <span className="font-semibold text-foreground">{selectedTask.title}</span></p>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-2">Add Image (optional)</label>
                <label className="w-full bg-surface border border-dashed border-border rounded-lg px-4 py-6 text-center cursor-pointer hover:border-primary/50 transition-colors group">
                  ...
                  <p className="text-sm text-muted-foreground">{proofImage ? proofImage.name : 'Click to upload'}</p>
                  <input
                    type="file"
                    accept="image/*"
              ...
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-2">Proof Notes</label>
                <textarea
                  value={proofText}
